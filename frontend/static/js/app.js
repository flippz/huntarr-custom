    ...
    applyRoleBasedUI: function () {
        fetch('./api/requestarr/users/me', { cache: 'no-store' })
            ...
            .then(data => {
                ...
                // Expose assigned categories for non-owner instance locking
                // Use resolved values which convert bundle:X to appType:instanceName
                window._huntarrUserMovieCategory = data.user.resolved_movie_category || '';
                window._huntarrUserTVCategory = data.user.resolved_tv_category || '';
