...
def test_library_delete_sets_activity_job_library_id_null(library_repo, activity_repo):
    """FK behavior: deleting a library must not delete its jobs - it
    only clears the reference (ON DELETE SET NULL), matching the
    previous SQLite schema."""
    library = library_repo.create(
        {"name": "Sonarr", "type": "sonarr", "url": "http://sonarr", "api_key": "k", "enabled": True}
    )
    job = activity_repo.create(
        {"library_id": library.id, "library_name": "Sonarr", "state": "completed", "title": "Scan"}
    )

    library_repo.delete(library.id)

    surviving = activity_repo.get(job.id)
    assert surviving is not None
    assert surviving.library_id is None


def test_deleting_activity_job_cascades_to_scan_candidates(database, activity_repo, candidate_repo):
    """FK behavior: deleting a job must delete its candidate snapshot
    rows (ON DELETE CASCADE), matching the previous SQLite schema."""
    job = activity_repo.create(
        {"library_id": None, "library_name": "Sonarr", "state": "completed", "title": "Scan"}
    )
    ...
    assert candidate_repo.count_for_job(job.id) == 1

    with database.connect() as conn:
        conn.execute("DELETE FROM activity_jobs WHERE id = %s", (job.id,))

    assert candidate_repo.count_for_job(job.id) == 0
