...
def test_activity_get_not_found(client):
    resp = client.get("/api/v1/activity/999")
...
def test_activity_reflects_repository_writes(app, client):
    ...
    repo = ActivityRepository(app.extensions["managearr"]["db"])
    job = repo.create(
