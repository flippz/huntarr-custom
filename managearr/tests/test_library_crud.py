...
def test_create_library_success(client):
    resp = client.post("/api/v1/libraries", json=create_payload())
...
def test_create_library_rejects_bad_type(client):
    resp = client.post("/api/v1/libraries", json=create_payload(type="not-an-arr"))
    assert resp.status_code == 400
    assert "type" in resp.get_json()["errors"][0]


def test_create_library_rejects_missing_fields(client):
    resp = client.post("/api/v1/libraries", json={"name": "Only a name"})
    assert resp.status_code == 400
...
def test_create_library_rejects_bad_url(client):
    resp = client.post("/api/v1/libraries", json=create_payload(url="not-a-url"))
    assert resp.status_code == 400
...
def test_get_library_not_found(client):
    resp = client.get("/api/v1/libraries/999")
    assert resp.status_code == 404


def test_update_library_partial(client):
    created = client.post("/api/v1/libraries", json=create_payload()).get_json()["library"]
    resp = client.put(f"/api/v1/libraries/{created['id']}", json={"enabled": False})
...
def test_update_library_not_found(client):
    resp = client.put("/api/v1/libraries/999", json={"enabled": False})
    assert resp.status_code == 404


def test_update_library_rejects_invalid_value(client):
    created = client.post("/api/v1/libraries", json=create_payload()).get_json()["library"]
    resp = client.put(f"/api/v1/libraries/{created['id']}", json={"type": "bogus"})
    assert resp.status_code == 400
...
def test_delete_library(client):
    ...
    resp = client.get(f"/api/v1/libraries/{created['id']}")
    assert resp.status_code == 404


def test_delete_library_not_found(client):
    resp = client.delete("/api/v1/libraries/999")
    assert resp.status_code == 404
