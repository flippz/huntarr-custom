...
def check_state_reset(app_type: str) -> bool:
    ...
    if hours_passed >= reset_interval:
        logger.info(f"[State] Resetting state for {app_type} after {hours_passed:.1f}h (interval: {reset_interval}h)")
        logger.warning(f"[State] All previously processed media for {app_type} will be eligible for reprocessing")
