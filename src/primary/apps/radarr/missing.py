...
def process_missing_movies(
    ...
    for movie in movies_to_process:
        ...
        if search_success:
            ...
            _entry_id = log_processed_media("radarr", media_name, tmdb_id, instance_key, "missing", display_name_for_log=app_settings.get("instance_display_name") or instance_name)
            ...
            if _entry_id:
                ...
                if _cmd_ok:
                    ...
                    mark_command(search_success, 'grabbed' if _grabbed else 'no_grab',
                                 cooldown_seconds=None if _grabbed else 300)
                else:
                    update_history_status(_entry_id, 'failed')
            
            increment_media_stat_only("radarr", "hunted", 1, instance_key)
            movies_processed += 1
            processed_any = True
        else:
